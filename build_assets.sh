...
TARGET="ArtRes"
ModuleName="LoveRoom"
BUILD=$ROOT/AssetBundleOutput/$platform
...
upload_files () {
	target_dir=$REPO/${TARGET}/${ModuleName}/${platform}
    ...
    DATA={\"env\":\"${ENV}\",\"target\":\"${TARGET}/${ModuleName}/${platform}\"}
    curl -X POST -H "Content-Type: application/json" http://172.16.2.3:5000/utown/assets/update -d $DATA
}

get_version() {
	cd ${BUILD}
	version_paths=`ls *.hash`
	version_path=${version_paths[0]}
	VERSION=${version_path#*_}
	VERSION=${VERSION%.*}
	echo addressable version:$VERSION
}
